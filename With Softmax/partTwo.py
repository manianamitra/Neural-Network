...
neuralNetDescription = [-1,100,50,2] #Only nmber of neurons no bias
...
def softmax(x):
	# Compute the softmax of vector x.
        exps = np.exp(x)
        numberOfRows = exps.shape[0]
        for iterator in range (0,numberOfRows):   #Handling Overflow
         if ( exps[iterator,0] == float('Inf')):
             exps[iterator,0] = sys.maxint
...
def backwardPass(correctPredictionSpam,correctPredictionHam):
    global deltaDict
    deltaDict = {} #Reinitializing delta vector
    lastLayerValue = learnedFeaturesDict.get(TOTAL_LAYERS-1)  
    
    predictedValueSpam = lastLayerValue[1,0]
    predictedValueHam = lastLayerValue[2,0]
    
    deltaSpam = predictedValueSpam - correctPredictionSpam
    deltaHam =  predictedValueHam - correctPredictionHam

    
    deltaVector = np.array([[0],[deltaSpam],[deltaHam]])
...
def inSampleError():
	...
	forwardPass(inputVector)
        ...
        predictedValueSpam = vectorAtLastLayer[1,0]
        predictedValueHam = vectorAtLastLayer[2,0]
        #print vectorAtLastLayer
        if (predictedValueSpam > predictedValueHam):
            error = error + 0
        else:
            error = error + 1
...
def trainNeuralNetwork():
    ...
    for noOfEpoc in range(0,NUMBER_OF_EPOC):
	    ##### Train For Spam Dataset ##################
            ...
            if len(listOfSpamVectors) < len(listOfHamVectors):
               minLength = len(listOfSpamVectors)
            else:
               minLength = len(listOfHamVectors)
 
	    for iterator in range(0,minLength):
                inputVectorList = [BIAS] + listOfSpamVectors[iterator]
                inputVector = np.array([inputVectorList])
		inputVector = inputVector.transpose()
		forwardPass(inputVector)
		backwardPass(1,0) 
		    ...
		    updateWeight(layer)
                ...
                inputVectorList = [BIAS] + listOfHamVectors[iterator]
                inputVector = np.array([inputVectorList])
		inputVector = inputVector.transpose()
		forwardPass(inputVector)
		backwardPass(0,1) 
            ...
            print "Training Done For same number of spam and ham messages"
            if (iterator == len(listOfSpamVectors)):
                print 'Spam are being trained.'
                for secondIterator in range(iterator,len(listOfHamVectors)):
                    inputVectorList = [BIAS] + listOfHamVectors[secondIterator]
                    inputVector = np.array([inputVectorList])
		    inputVector = inputVector.transpose()
		    forwardPass(inputVector)
		    backwardPass(0,1) 
               ...
               print 'Ham are being trained.'
               for secondIterator in range(iterator,len(listOfSpamVectors)):
                    inputVectorList = [BIAS] + listOfSpamVectors[secondIterator]
                    inputVector = np.array([inputVectorList])
		    inputVector = inputVector.transpose()
		    forwardPass(inputVector)
		    backwardPass(1,0) 
            ...
            print 'Training Done on rest of the data set.'
